    ...
    <header className="sticky top-0 z-30 border-b border-zinc-200 bg-white/85 backdrop-blur-md dark:border-zinc-800 dark:bg-zinc-950/85">
      <div className="mx-auto w-full max-w-6xl px-4 sm:px-6">
        <div className="flex h-14 items-center justify-between gap-4">
          <Link
            href="/"
            className="fokus-cincin flex items-center gap-2.5 rounded-md"
            aria-label="Pusaka, halaman utama"
          >
            <Tanda />
            <span className="flex flex-col leading-none">
              <span className="text-base font-semibold tracking-tight">Pusaka</span>
          ...
          <nav aria-label="Navigasi utama" className="hidden items-center gap-1 sm:flex">
            {NAV.map((n) => (
              <Link
                ...
                className={`fokus-cincin rounded-md px-3 py-1.5 text-sm transition ${
                  aktif(n.href)
            ...
            <a
              ...
              target="_blank"
              className="fokus-cincin flex items-center gap-1.5 rounded-md px-3 py-1.5 text-sm text-zinc-600 transition hover:bg-zinc-100 hover:text-zinc-900 dark:text-zinc-400 dark:hover:bg-zinc-800 dark:hover:text-zinc-100"
            >
              <LogoGithub />
              GitHub
            </a>
          ...
          <button
            ...
            className="fokus-cincin -mr-1 flex h-9 w-9 items-center justify-center rounded-md text-zinc-600 transition hover:bg-zinc-100 sm:hidden dark:text-zinc-400 dark:hover:bg-zinc-800"
          >
        ...
        {menuTerbuka && (
          <div id="menu-ponsel" className="border-t border-zinc-200 py-3 sm:hidden dark:border-zinc-800">
            <nav aria-label="Navigasi utama ponsel" className="flex flex-col gap-1">
              {DAFTAR_META.map((a) => (
                <Link
                  key={a.slug}
                  href={`/alat/${a.slug}`}
                  className="fokus-cincin flex items-center gap-2.5 rounded-md px-2 py-2 text-sm transition hover:bg-zinc-100 dark:hover:bg-zinc-800"
                >
              ...
              <a
                ...
                rel="noopener noreferrer"
                target="_blank"
                className="fokus-cincin flex items-center gap-2 rounded-md px-2 py-2 text-sm transition hover:bg-zinc-100 dark:hover:bg-zinc-800"
              >
                <LogoGithub />
                GitHub
              </a>
